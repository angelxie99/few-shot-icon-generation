...
max_train_steps=300 # change accordingly
...
unet.load_state_dict(torch.load('unet-1.pt'))
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example["unconditional_prompt"] = self.tokenizer(
            "a custom icon",
            padding="do_not_pad",
            truncation=True,
            max_length=self.tokenizer.model_max_length,
...
def training_function(text_encoder, vae, unet, batch_size, with_prior, prior_weight,  train_dataloader ):
    ...
    unconditional_encoder_hidden_states = None
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    conditional_encoder_hidden_states = text_encoder(batch["input_ids"])[0]
                    if(unconditional_encoder_hidden_states is None):
                        unconditional_encoder_hidden_states = text_encoder(batch["input_ids"])[0]
                        #unconditional_encoder_hidden_states.requires_grad_(True)

                    print(unconditional_encoder_hidden_states.shape)
                    print(conditional_encoder_hidden_states.shape)

                # Predict the noise residual with conditional gan

                uncon_noise_pred = unet(noisy_latents, timesteps, unconditional_encoder_hidden_states).sample
                noise_pred = unet(noisy_latents, timesteps, conditional_encoder_hidden_states).sample
                #noise_pred = uncon_noise_pred
                ...
                if with_prior:
                    ...
                    loss = F.mse_loss(uncon_noise_pred + 0.7 * (noise_pred - uncon_noise_pred), noise, reduction="none").mean([1, 2, 3]).mean()
...
def db_eval(instance_prompts, prior_preservation, prior_preservation_class_prompt, 
    ...
    def collate_fn(examples):
        ...
        unconditional_input_ids = [example["unconditional_prompt"] for example in examples]
        ...
        unconditional_input_ids = tokenizer.pad({"input_ids": unconditional_input_ids}, padding=True, return_tensors="pt").input_ids
...
if __name__ == '__main__':
    ...
    # change accordingly for the following variables
    # ==============================================
    instance_prompts = ["a custom icon"]*10 # change accordingly
    ...
    output_image_dir = "./output_images/output_images_300_conditional" # change accordingly
